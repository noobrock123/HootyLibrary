...
def user_id_gen():
    id = rand.randint(1, (1 << 31) - 1)
    try:
        i = 1
        while User.objects.get(user_id=id) != None:
            id = (id + i*i) % ((1 << 16) - 1)
    except:
        pass
    return id
...
class CustomAccountManager(BaseUserManager):
    ...
    def create_user(self, username, email, password, **other_fields):
        ...
        if len(username) == 0:
            username = id
        user = self.model(user_id=id, username=username,
...
def user_profile_pic_path(obj, file):
    return f"{settings.MEDIA_ROOT}/{obj.user_id}/profile_pic/{file}"


class User(AbstractBaseUser, PermissionsMixin):
    user_id = models.TextField(
        primary_key=True, max_length=12, default=user_id_gen)
    ...
    profile_pic = models.ImageField(
        upload_to=user_profile_pic_path, null=True, blank=True)
...
def book_thumbnail_pic_path(obj, file):
    return f"{settings.MEDIA_ROOT}/{obj.book_id}/thumbnail/{file}"


def book_pdf_files_path(obj, file):
    return f"{settings.MEDIA_ROOT}/{obj.book_id}/pdf_files/{file}"
...
class Book(models.Model):
    ...
    thumbnail = models.ImageField(
        upload_to=book_thumbnail_pic_path, blank=True, null=True)
    pdf_files = models.FileField(
        upload_to=book_pdf_files_path, blank=True, null=True)
