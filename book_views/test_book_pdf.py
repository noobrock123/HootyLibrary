...
class BookPDFTestCase(TestCase):
    def setUp(self) -> None:
    ...
    def test_client_can_access_book_pdf_set(self):
        # test client can access book's pdf not set
        c = Client()
        response = c.get(f'/book/{self.book3.book_id}/pdf/')
        with self.subTest():
            self.assertEqual(response.status_code, 200)
        with self.subTest():
            with open(f'book_views/test_data/RH_StudyGuide_V2.pdf', 'rb') as pdf:
                self.assertEqual(pdf.read(), response.content)

    def test_client_access_book_pdf_not_set(self):
        # test client access book's pdf not set
        c = Client()
        response = c.get(f'/book/{self.book1.book_id}/pdf/')
        with self.subTest():
